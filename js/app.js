(() => {
  ...
  async function bootstrapUser() {
    const [pSnap, rSnap, oSnap] = await Promise.all([
  ...
  async function people(v) {
    ...
    const faculty = Object.entries(us).filter(([uid]) => ["instructor","admin"].includes(rs[uid]));
    const students = Object.entries(us).filter(([uid]) => !rs[uid] || rs[uid] === "student");
    const accountRow = ([uid,p]) => `<div class="item"><div><b>${esc(p.displayName || p.email)}</b><div class="muted">${esc(p.email || "")} ${p.department ? `· ${esc(p.department)}` : ""}</div></div><div><span class="tag">${esc(rs[uid] || "student")}</span>${uid !== user.uid ? ` <button class="btn ghost" data-role-uid="${uid}" data-current="${esc(rs[uid] || "student")}">Manage</button>` : ""}</div></div>`;
